...
const App_old: React.FC = () => {
  ...
  const mutation = useMutation(
    async (data: {book: number; chapter: number}) => {
        ...
        case 'verse':
          ...
          if (verseData) {
            window.open(
              verseData[2],
              '',
              'left=600,top=250,width=700,height=700',
            );
  ...
  const setBookInput = () => {
    const bookObj = books.find(
      (book) =>
        book[0].toUpperCase() === bookString.toUpperCase() ||
        book[5].toUpperCase().replace('.', '') === bookString.toUpperCase(),
    );
    if (bookObj) {
      setBook(bookObj[1]);
    }
  ...
  return (
    <div className="App">
      <header>
        <span className={'book'}>
          {book > 0
            ? `${books[book - 1][0]} ${chapter > 0 ? chapter : ''}`
            : 'Biblia'}
        </span>
      ...
      <div className={'grid'}>
        ...
        {/* Lista wersetów */}
        {chapter > 0 && <>{renderList(verseCount, 'verse')}</>}
      </div>
      <div className={'grid'}>
        {/* Nowy testament */}
        {!book && (
